...
export function ScanEntry({graders,onConfirm,onManual,onManualDetails,onCancel}: Props) {
  ...
  useEffect(() => {
    ...
    const current = startCamera({video:video.current,onRead(raw) {
      ...
      setStatus('Code found'); setPayload(parsed); setCert(classification.kind==='grader-certification-url'?classification.payload.certNumber??'':parsed.certNumber ?? ''); setGrader(''); setHasTorch(false);
