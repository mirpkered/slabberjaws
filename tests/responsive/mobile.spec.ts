...
for (const [width,height] of sizes) {
  test(`${width}px collection and nested dialogs fit`, async ({page}, testInfo) => {
    ...
    await page.locator('.desktop-add').click();
    await page.getByLabel('Grading company').selectOption('CSG');
    await page.getByLabel('Certification number').fill('1012833027');
    await expect(page.getByRole('button',{name:'Continue to card details'})).toBeEnabled();
    await page.getByRole('button',{name:'Continue to card details'}).click();
    await expect(page.locator('.identity code')).toHaveText('1012833027');
    await expect(page.locator('.lookup-failed')).toHaveCount(0);
    await fits(page);
    await page.locator('.add-modal .close').click();
    await page.locator('.desktop-add').click();
    await expect(page.getByLabel('Grading company')).toHaveValue('Degree');
    await expect(page.getByLabel('Certification number')).toHaveValue('');
    await page.getByRole('button',{name:'Photograph Slab'}).click();
    await page.getByLabel('Grading company').selectOption('C3G');
    await page.locator('.add-modal .close').click();
    await page.locator('.desktop-add').click();
    await expect(page.getByLabel('Grading company')).toHaveValue('Degree');
    await expect(page.getByLabel('Certification number')).toHaveValue('');
